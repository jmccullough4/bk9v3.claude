...
function initWebSocket() {
    ...
    // HackRF WebSocket events
    socket.on('hackrf_spectrum_update', (data) => {
        handleHackrfSpectrumUpdate(data);
    });

    socket.on('hackrf_piconet_detected', (data) => {
        handleHackrfPiconetDetected(data);
    });

    socket.on('hackrf_target_detected', (data) => {
        addLogEntry(`HackRF target detected: ${data.bd_address} (${data.active_channels}/${data.total_channels} channels active)`, 'INFO');
    });
...
function updateRadioListSettings(radios) {
    ...
    // HackRF SDR devices
    const hackrfList = radios.hackrf || [];
    hackrfList.forEach(radio => {
        const item = document.createElement('div');
        const statusClass = radio.status === 'running' ? 'status-up' : (radio.status === 'ready' ? '' : 'status-down');
        item.className = `radio-item hackrf-radio`;
        item.innerHTML = `
            <div class="item-info">
                <span class="item-primary">${radio.interface} (HackRF SDR)</span>
                <span class="item-secondary">FW: ${radio.firmware || 'Unknown'} | SN: ${radio.serial?.slice(-8) || 'Unknown'}</span>
                <span class="item-status ${statusClass}">${radio.status.toUpperCase()}</span>
            </div>
            <div class="item-capabilities">
                ${radio.capabilities?.map(cap => `<span class="cap-badge">${cap}</span>`).join('') || ''}
            </div>
        `;
        list.appendChild(item);
    });
...
let hackrfRunning = false;
let hackrfSpectrumCtx = null;
let hackrfWaterfallCtx = null;
let hackrfWaterfallData = [];
let hackrfSpectrumData = [];
let hackrfChannelData = [];

/**
 * Initialize HackRF UI components
 */
function initHackrfUI() {
    // Initialize spectrum canvas
    const spectrumCanvas = document.getElementById('hackrfSpectrumCanvas');
    if (spectrumCanvas) {
        hackrfSpectrumCtx = spectrumCanvas.getContext('2d');
        const parent = spectrumCanvas.parentElement;
        if (parent) {
            spectrumCanvas.width = parent.offsetWidth - 20;
        }
    }

    // Initialize waterfall canvas
    const waterfallCanvas = document.getElementById('hackrfWaterfallCanvas');
    if (waterfallCanvas) {
        hackrfWaterfallCtx = waterfallCanvas.getContext('2d');
        const parent = waterfallCanvas.parentElement;
        if (parent) {
            waterfallCanvas.width = parent.offsetWidth - 20;
        }
    }

    // Initialize channel grid
    initHackrfChannelGrid();

    // Setup config sliders
    setupHackrfConfigSliders();
...
/**
 * Initialize the 79-channel activity grid
 */
function initHackrfChannelGrid() {
    const grid = document.getElementById('hackrfChannelGrid');
    if (!grid) return;
    ...
    for (let i = 0; i < 79; i++) {
        ...
        cell.id = `hackrf-ch-${i}`;
        cell.title = `Ch ${i} (${2402 + i} MHz)`;
        cell.innerHTML = `<span class="ch-num">${i}</span>`;
        grid.appendChild(cell);
    }
}

/**
 * Setup config slider event listeners
 */
function setupHackrfConfigSliders() {
    const lnaSlider = document.getElementById('hackrfLnaGain');
    const vgaSlider = document.getElementById('hackrfVgaGain');
    const thresholdSlider = document.getElementById('hackrfThreshold');

    if (lnaSlider) {
        lnaSlider.addEventListener('input', () => {
            document.getElementById('hackrfLnaGainValue').textContent = `${lnaSlider.value} dB`;
        });
        lnaSlider.addEventListener('change', updateHackrfConfig);
    }

    if (vgaSlider) {
        vgaSlider.addEventListener('input', () => {
            document.getElementById('hackrfVgaGainValue').textContent = `${vgaSlider.value} dB`;
        });
        vgaSlider.addEventListener('change', updateHackrfConfig);
    }

    if (thresholdSlider) {
        thresholdSlider.addEventListener('input', () => {
            document.getElementById('hackrfThresholdValue').textContent = `${thresholdSlider.value} dBm`;
        });
        thresholdSlider.addEventListener('change', updateHackrfConfig);
    }
}

/**
 * Load HackRF status from API
 */
function loadHackrfStatus() {
    fetch('/api/hackrf/status')
        ...
        .catch(error => {
            console.error('Error loading HackRF status:', error);
            const statusText = document.getElementById('hackrfStatusText');
            if (statusText) {
...
/**
 * Update HackRF UI with status data
 */
function updateHackrfUI(data) {
    const statusText = document.getElementById('hackrfStatusText');
    const firmware = document.getElementById('hackrfFirmware');
    const activeChannels = document.getElementById('hackrfActiveChannels');
    const hopPatterns = document.getElementById('hackrfHopPatterns');
    const startBtn = document.getElementById('hackrfStartBtn');
    const stopBtn = document.getElementById('hackrfStopBtn');

    if (data.available) {
        if (statusText) {
            statusText.textContent = data.running ? 'Running' : 'Ready';
            statusText.className = 'status-value ' + (data.running ? 'running' : 'ready');
        }
        if (firmware && data.devices && data.devices[0]) {
            firmware.textContent = data.devices[0].firmware || 'Unknown';
        }
    } else {
        if (statusText) {
            statusText.textContent = data.error || 'Not Available';
    ...
    if (activeChannels) {
        activeChannels.textContent = `${data.active_channels || 0}/79`;
    }
    if (hopPatterns) {
        hopPatterns.textContent = data.hop_patterns_detected || 0;
    }

    hackrfRunning = data.running || false;

    if (startBtn) startBtn.disabled = hackrfRunning;
    if (stopBtn) stopBtn.disabled = !hackrfRunning;
}

/**
 * Start HackRF spectrum sweep
 */
function startHackrf() {
    fetch('/api/hackrf/start', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ mode: 'spectrum' })
    })
        .then(response => response.json())
        .then(data => {
            if (data.status === 'started') {
                addLogEntry('HackRF spectrum sweep started', 'INFO');
                hackrfRunning = true;
                loadHackrfStatus();
                addOperation('hackrf-sweep', 'SPECTRUM', 'HackRF Sweep', {
                    cancellable: true,
                    cancelFn: stopHackrf
                });
            } else {
                addLogEntry(`Failed to start HackRF: ${data.message}`, 'ERROR');
            }
        })
        .catch(error => {
            addLogEntry('Error starting HackRF: ' + error, 'ERROR');
        });
}
 ...
 */
function stopHackrf() {
    fetch('/api/hackrf/stop', { method: 'POST' })
        .then(response => response.json())
        .then(data => {
            if (data.status === 'stopped') {
                addLogEntry('HackRF spectrum sweep stopped', 'INFO');
                hackrfRunning = false;
                loadHackrfStatus();
                removeOperation('hackrf-sweep');
            } else {
                addLogEntry(`Failed to stop HackRF: ${data.message}`, 'ERROR');
            }
        })
        .catch(error => {
            addLogEntry('Error stopping HackRF: ' + error, 'ERROR');
        });
}
...
/**
 * Load spectrum data from API
 */
function loadHackrfSpectrum() {
    fetch('/api/hackrf/spectrum')
        .then(response => response.json())
        .then(data => {
            hackrfSpectrumData = data.spectrum || [];
            renderHackrfSpectrum(data);
            updateHackrfChannelGrid(data.spectrum);

            const peakPower = document.getElementById('hackrfPeakPower');
            const utilization = document.getElementById('hackrfUtilization');
            const activeChannels = document.getElementById('hackrfActiveChannels');

            if (peakPower) peakPower.textContent = `${data.peak_power?.toFixed(1) || '--'} dBm`;
            if (utilization) utilization.textContent = `${data.channel_utilization?.toFixed(1) || 0}%`;
            if (activeChannels) activeChannels.textContent = `${data.active_channels || 0}/79`;
        })
        .catch(error => {
            console.error('Error loading HackRF spectrum:', error);
        });
}

/**
 * Render spectrum visualization on canvas
 */
function renderHackrfSpectrum(data) {
    if (!hackrfSpectrumCtx) return;

    const canvas = hackrfSpectrumCtx.canvas;
    const width = canvas.width;
    const height = canvas.height;
    const spectrum = data.spectrum || [];

    // Clear canvas
    hackrfSpectrumCtx.fillStyle = '#0a0e14';
    hackrfSpectrumCtx.fillRect(0, 0, width, height);

    // Draw grid
    hackrfSpectrumCtx.strokeStyle = 'rgba(0, 212, 255, 0.1)';
    hackrfSpectrumCtx.lineWidth = 1;

    for (let i = 0; i <= 10; i++) {
        const y = (i / 10) * height;
        hackrfSpectrumCtx.beginPath();
        hackrfSpectrumCtx.moveTo(0, y);
        hackrfSpectrumCtx.lineTo(width, y);
        hackrfSpectrumCtx.stroke();
    }

    for (let i = 0; i <= 79; i += 10) {
        const x = (i / 79) * width;
        hackrfSpectrumCtx.beginPath();
        hackrfSpectrumCtx.moveTo(x, 0);
        hackrfSpectrumCtx.lineTo(x, height);
        hackrfSpectrumCtx.stroke();
    }

    if (spectrum.length === 0) return;

    const barWidth = width / 79;
    const threshold = parseInt(document.getElementById('hackrfThreshold')?.value || -60);

    spectrum.forEach((ch, i) => {
        const x = i * barWidth;
        const normalizedPower = Math.max(0, Math.min(1, (ch.power + 100) / 100));
        const barHeight = normalizedPower * height;

        let color;
        if (ch.power > threshold) {
            color = 'rgba(48, 209, 88, 0.8)';
        } else if (ch.power > threshold - 20) {
            color = 'rgba(255, 176, 0, 0.6)';
        } else {
            color = 'rgba(0, 212, 255, 0.3)';
        }

        hackrfSpectrumCtx.fillStyle = color;
        hackrfSpectrumCtx.fillRect(x, height - barHeight, barWidth - 1, barHeight);
    });

    // Draw threshold line
    const thresholdY = height - ((threshold + 100) / 100) * height;
    hackrfSpectrumCtx.strokeStyle = 'rgba(255, 59, 48, 0.8)';
    hackrfSpectrumCtx.lineWidth = 2;
    hackrfSpectrumCtx.setLineDash([5, 5]);
    hackrfSpectrumCtx.beginPath();
    hackrfSpectrumCtx.moveTo(0, thresholdY);
    hackrfSpectrumCtx.lineTo(width, thresholdY);
    hackrfSpectrumCtx.stroke();
    hackrfSpectrumCtx.setLineDash([]);
}

/**
 * Update channel activity grid
 */
function updateHackrfChannelGrid(spectrum) {
    if (!spectrum) return;

    const threshold = parseInt(document.getElementById('hackrfThreshold')?.value || -60);

    spectrum.forEach((ch, i) => {
        const cell = document.getElementById(`hackrf-ch-${i}`);
        if (!cell) return;

        cell.classList.remove('active', 'moderate', 'inactive');
...
/**
 * Load detected piconets/patterns
 */
function loadHackrfPiconets() {
    fetch('/api/hackrf/hop_patterns')
        .then(response => response.json())
        .then(data => {
            updateHackrfPiconetsTable(data.patterns || []);
            const hopPatterns = document.getElementById('hackrfHopPatterns');
            if (hopPatterns) hopPatterns.textContent = data.count || 0;
        })
        .catch(error => {
            console.error('Error loading HackRF piconets:', error);
        });
}

/**
 * Update piconets table
 */
function updateHackrfPiconetsTable(patterns) {
    const tableBody = document.getElementById('hackrfPiconetsTable');
    if (!tableBody) return;

    if (patterns.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="7" class="empty-row">No patterns detected</td></tr>';
        return;
    }

    tableBody.innerHTML = patterns.map(pattern => `
        <tr class="${pattern.is_bluetooth_like ? 'bt-like' : ''}">
            <td class="pattern-id">${pattern.pattern_id}</td>
            <td>${pattern.channel_count} channels</td>
            <td>${pattern.avg_power?.toFixed(1) || '--'} dBm</td>
            <td>${pattern.is_bluetooth_like ? '<span class="badge bt">BT</span>' : '<span class="badge other">Other</span>'}</td>
            <td>${pattern.correlated_lap || '--'}</td>
            <td>${pattern.detected_at ? new Date(pattern.detected_at).toLocaleTimeString() : '--'}</td>
            <td>
                <button class="btn btn-sm btn-secondary" onclick="viewPatternDetails('${pattern.pattern_id}')">View</button>
            </td>
        </tr>
    `).join('');
}
 ...
 * Clear HackRF data
 */
function clearHackrfData() {
    if (!confirm('Clear all captured HackRF data?')) return;

    fetch('/api/hackrf/clear', { method: 'POST' })
        .then(response => response.json())
        .then(data => {
            if (data.status === 'cleared') {
                addLogEntry('HackRF data cleared', 'INFO');
                hackrfSpectrumData = [];
                hackrfChannelData = [];
                hackrfWaterfallData = [];
                refreshHackrfData();
        ...
        .catch(error => {
            addLogEntry('Error clearing HackRF data: ' + error, 'ERROR');
        });
}

/**
 * Export HackRF capture data
 */
function exportHackrfData() {
    fetch('/api/hackrf/export', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({})
    })
        ...
        .then(data => {
            if (data.status === 'exported') {
                addLogEntry(`HackRF data exported to ${data.filename}`, 'INFO');
                alert(`Data exported to: ${data.filepath}`);
            } else {
                addLogEntry('Error exporting HackRF data: ' + data.message, 'ERROR');
            }
        })
        .catch(error => {
            addLogEntry('Error exporting HackRF data: ' + error, 'ERROR');
        });
}
 ...
 */
function updateHackrfConfig() {
    const lnaGain = parseInt(document.getElementById('hackrfLnaGain')?.value || 32);
    const vgaGain = parseInt(document.getElementById('hackrfVgaGain')?.value || 20);
    const threshold = parseInt(document.getElementById('hackrfThreshold')?.value || -60);

    fetch('/api/hackrf/config', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            LNA_GAIN: lnaGain,
        ...
        .then(data => {
            if (data.status === 'updated') {
                addLogEntry('HackRF config updated', 'INFO');
            }
        })
        .catch(error => {
            console.error('Error updating HackRF config:', error);
        });
}

/**
 * Toggle waterfall display visibility
 */
function toggleWaterfall() {
    const waterfall = document.getElementById('hackrfWaterfallDisplay');
    if (waterfall) {
...
function loadFusionData() {
    fetch('/api/hackrf/fusion')
        .then(response => response.json())
        .then(data => {
            updateFusionDisplay(data);
        })
        .catch(error => {
            addLogEntry('Error loading fusion data: ' + error, 'ERROR');
        });
}

/**
 * Update fusion intelligence display
 */
function updateFusionDisplay(data) {
    const sourcesActive = document.getElementById('fusionSourcesActive');
    const correlatedDevices = document.getElementById('fusionCorrelatedDevices');
    const recommendations = document.getElementById('fusionRecommendations');

    if (sourcesActive) {
        const activeCount = (data.sources?.hackrf ? 1 : 0) +
                           (data.sources?.ubertooth ? 1 : 0) +
                           (data.sources?.bt_adapters > 0 ? 1 : 0);
        sourcesActive.textContent = `${activeCount}/3`;
    }

    if (correlatedDevices) {
        const correlated = data.device_correlations?.filter(d => d.hackrf_correlation).length || 0;
        correlatedDevices.textContent = correlated;
    }

    if (recommendations && data.recommendations) {
        if (data.recommendations.length === 0) {
            recommendations.innerHTML = '<p class="no-recommendations">All systems operating optimally</p>';
        } else {
...
function getRecommendationIcon(type) {
    switch (type) {
        case 'enable_scanner': return '&#128225;';
        case 'enable_ubertooth': return '&#128301;';
        case 'high_activity': return '&#128200;';
        default: return '&#128161;';
    }
}
    ...
    fetch('/api/hackrf/hop_patterns')
        ...
        .then(data => {
            ...
            if (pattern) {
                ...
                alert(`Pattern ${patternId}\n\nChannels (${channels.length}): ${channels.slice(0, 20).join(', ')}${channels.length > 20 ? '...' : ''}\n\nAvg Power: ${pattern.avg_power?.toFixed(1)} dBm\nBluetooth-like: ${pattern.is_bluetooth_like ? 'Yes' : 'No'}\nCorrelated LAP: ${pattern.correlated_lap || 'None'}`);
            }
        });
}

/**
 * Handle HackRF spectrum update from WebSocket
 */
function handleHackrfSpectrumUpdate(data) {
    hackrfSpectrumData = data.spectrum || [];
    renderHackrfSpectrum(data);
    updateHackrfChannelGrid(data.spectrum);
    ...
    if (!document.getElementById('hackrfWaterfallDisplay')?.classList.contains('hidden')) {
...
function updateWaterfall(spectrum) {
    if (!hackrfWaterfallCtx) return;

    const canvas = hackrfWaterfallCtx.canvas;
    const width = canvas.width;
    const height = canvas.height;

    // Scroll existing image down
    const imageData = hackrfWaterfallCtx.getImageData(0, 0, width, height - 1);
    hackrfWaterfallCtx.putImageData(imageData, 0, 1);

    const threshold = parseInt(document.getElementById('hackrfThreshold')?.value || -60);

    spectrum.forEach((ch, i) => {
        const x = Math.floor((i / 79) * width);
        const pixelWidth = Math.ceil(width / 79);

        const normalizedPower = Math.max(0, Math.min(1, (ch.power + 100) / 70));
        const hue = (1 - normalizedPower) * 240;
        hackrfWaterfallCtx.fillStyle = `hsl(${hue}, 100%, ${30 + normalizedPower * 40}%)`;
        hackrfWaterfallCtx.fillRect(x, 0, pixelWidth, 1);
    });
}

/**
 * Handle HackRF piconet detected event
 */
function handleHackrfPiconetDetected(data) {
    addLogEntry(`HackRF detected potential piconet: ${data.pattern_id} (${data.channel_count} channels)`, 'INFO');
    loadHackrfPiconets();

    const hopPatterns = document.getElementById('hackrfHopPatterns');
    if (hopPatterns) {
        hopPatterns.textContent = parseInt(hopPatterns.textContent || '0') + 1;
    }
}
