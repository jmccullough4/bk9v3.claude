...
hackrf_sweep_data = {}  # frequency -> power level for spectrum display
hackrf_piconet_data = {}  # LAP -> piconet info detected via SDR
hackrf_channel_activity = {}  # BT channel (0-78) -> activity metrics
hackrf_hop_sequences = {}  # Detected frequency hopping patterns
hackrf_raw_captures = []  # Recent raw captures for analysis

# HackRF Configuration
HACKRF_CONFIG = {
    'FREQ_START': 2402000000,  # Bluetooth channel 0 (2402 MHz)
    'FREQ_STOP': 2480000000,   # Bluetooth channel 78 (2480 MHz)
    'FREQ_BIN_WIDTH': 100000,  # 100 kHz bins for fine resolution
    'LNA_GAIN': 32,            # Low Noise Amplifier gain (0-40 dB)
    'VGA_GAIN': 20,            # Variable Gain Amplifier (0-62 dB)
    'SAMPLE_RATE': 20000000,   # 20 MHz sample rate
    'SWEEP_STYLE': 'LINEAR',   # LINEAR or INTERLEAVED
    'ONE_SHOT': False,         # Continuous sweep
    'FFT_SIZE': 1024,          # FFT size for spectrum analysis
    # Bluetooth-specific settings
    'BT_CHANNEL_WIDTH': 1000000,  # 1 MHz per BT channel
    'BT_CHANNELS': 79,            # Bluetooth has 79 channels (0-78)
    'BT_BASE_FREQ': 2402000000,   # Channel 0 = 2402 MHz
    # Piconet detection thresholds
    'ACTIVITY_THRESHOLD': -60,    # dBm threshold for activity detection
    'HOP_DETECT_WINDOW': 1.0,     # Seconds to analyze for hopping
    'MIN_HOP_CHANNELS': 5,        # Minimum channels to consider hopping
}
...
def init_database():
    ...
    # HackRF spectrum captures
    c.execute('''CREATE TABLE IF NOT EXISTS hackrf_spectrum (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    # HackRF detected piconets/patterns
    c.execute('''CREATE TABLE IF NOT EXISTS hackrf_piconets (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        pattern_id TEXT UNIQUE,
        channels TEXT,
        channel_count INTEGER,
        avg_power REAL,
        is_bluetooth_like INTEGER DEFAULT 0,
        correlated_lap TEXT,
    ...
    # HackRF configuration history
    c.execute('''CREATE TABLE IF NOT EXISTS hackrf_config_history (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        config_json TEXT,
        changed_by TEXT,
...
def get_available_radios():
    """Get list of available Bluetooth, WiFi, Ubertooth, and HackRF radios."""
    radios = {'bluetooth': [], 'wifi': [], 'ubertooth': [], 'hackrf': []}
    ...
    try:
            ...
            if result.returncode == 0:
                ...
                for line in result.stdout.split('\n'):
                    ...
                    if 'Serial number:' in line:
                        ...
                        if serial:
                            radios['hackrf'].append({
                                'interface': f'hackrf{device_idx}',
                                'serial': serial,
                                'firmware': firmware or 'Unknown',
                                'board_id': board_id or 'Unknown',
                                'status': 'running' if hackrf_running else 'ready',
                                'type': 'hackrf',
                                'capabilities': ['spectrum_sweep', 'piconet_detect', 'bt_analysis']
                            })
                            device_idx += 1
        ...
        logger.debug(f"HackRF not available: {e}")
...
# ==================== HACKRF SDR FUNCTIONS ====================

def check_hackrf_available():
    """Check if HackRF tools are available on the system."""
    try:
        result = subprocess.run(['which', 'hackrf_sweep'], capture_output=True, text=True)
        return result.returncode == 0
    except Exception:
        return False
...
def get_hackrf_info():
    """Get HackRF device info and status."""
    try:
        result = subprocess.run(['hackrf_info'], capture_output=True, text=True, timeout=5)
        if result.returncode == 0:
            ...
            for line in output.split('\n'):
                ...
                if 'Serial number:' in line:
                    ...
                    current_device['board_id'] = line.split(':')[-1].strip()
                elif 'Firmware Version:' in line:
                    current_device['firmware'] = line.split(':')[-1].strip()
                elif 'Part ID Number:' in line:
                    current_device['part_id'] = line.split(':')[-1].strip()
        ...
        return {'available': False, 'error': 'HackRF tools not installed'}
    except subprocess.TimeoutExpired:
        return {'available': False, 'error': 'HackRF device not responding'}
    except Exception as e:
        return {'available': False, 'error': str(e)}
    return {'available': False, 'error': 'No HackRF device found'}


def freq_to_bt_channel(freq_hz):
    """Convert frequency in Hz to Bluetooth channel number (0-78)."""
    freq_mhz = freq_hz / 1000000
    if 2402 <= freq_mhz <= 2480:
        return int(freq_mhz - 2402)
    return -1


def bt_channel_to_freq(channel):
    """Convert Bluetooth channel (0-78) to frequency in Hz."""
    if 0 <= channel <= 78:
        return (2402 + channel) * 1000000
    return 0


def parse_hackrf_sweep_line(line):
    """
    Parse a line of hackrf_sweep output.
    Format: timestamp_ms, freq_start_hz, freq_stop_hz, bin_width_hz, num_samples, dB values...
    """
    global hackrf_sweep_data, hackrf_channel_activity
    ...
    try:
        ...
        bin_width = int(float(parts[3]))
        num_samples = int(parts[4])

        # Parse dB values
        db_values = [float(v) for v in parts[5:] if v.strip()]
        ...
        for i, db in enumerate(db_values):
            ...
            # Store in sweep data
            hackrf_sweep_data[freq] = {
                'power': db,
                'timestamp': timestamp / 1000.0,
                'bin_width': bin_width
            }

            # Check if this is a Bluetooth channel
            bt_channel = freq_to_bt_channel(freq)
            if bt_channel >= 0:
                if bt_channel not in hackrf_channel_activity:
                    hackrf_channel_activity[bt_channel] = {
                        'channel': bt_channel,
                        'frequency': bt_channel_to_freq(bt_channel),
                        'max_power': db,
                        'min_power': db,
                        'avg_power': db,
                        'samples': 1,
                        'active': db > HACKRF_CONFIG['ACTIVITY_THRESHOLD'],
                        'last_active': timestamp / 1000.0 if db > HACKRF_CONFIG['ACTIVITY_THRESHOLD'] else None,
                        'hop_count': 0
                    }
                else:
                    activity = hackrf_channel_activity[bt_channel]
                    activity['max_power'] = max(activity['max_power'], db)
                    activity['min_power'] = min(activity['min_power'], db)
                    # Running average
                    activity['avg_power'] = ((activity['avg_power'] * activity['samples']) + db) / (activity['samples'] + 1)
                    activity['samples'] += 1

                    was_active = activity['active']
                    activity['active'] = db > HACKRF_CONFIG['ACTIVITY_THRESHOLD']

                    if activity['active'] and not was_active:
                        activity['hop_count'] += 1
                        activity['last_active'] = timestamp / 1000.0
        ...
        logger.error(f"Error parsing hackrf_sweep output: {e}")
        return None


def detect_piconets_from_hopping():
    """
    Analyze channel activity to detect potential piconets based on
    frequency hopping patterns.
    """
    global hackrf_piconet_data, hackrf_channel_activity, hackrf_hop_sequences

    # Get channels with recent activity
    active_channels = []
    current_time = time.time()

    for ch, activity in hackrf_channel_activity.items():
        if activity['last_active'] and (current_time - activity['last_active']) < HACKRF_CONFIG['HOP_DETECT_WINDOW']:
            active_channels.append({
                'channel': ch,
                'hop_count': activity['hop_count'],
                'avg_power': activity['avg_power'],
                'last_active': activity['last_active']
            })

    # Sort by last active time to find hopping sequences
    active_channels.sort(key=lambda x: x['last_active'] if x['last_active'] else 0)

    # Look for patterns that suggest frequency hopping
    if len(active_channels) >= HACKRF_CONFIG['MIN_HOP_CHANNELS']:
        # Calculate hopping characteristics
        hop_pattern = {
            'channels': [c['channel'] for c in active_channels],
            'avg_dwell_time': HACKRF_CONFIG['HOP_DETECT_WINDOW'] / len(active_channels),
            'detected_at': datetime.now().isoformat(),
            'channel_count': len(active_channels),
            'avg_power': sum(c['avg_power'] for c in active_channels) / len(active_channels)
        }

        # Generate a pattern ID based on channels used
        pattern_id = hashlib.md5(str(sorted(hop_pattern['channels'])).encode()).hexdigest()[:8]

        # Check if this looks like Bluetooth frequency hopping
        # Bluetooth uses pseudo-random hopping across 79 channels
        is_bt_like = len(active_channels) > 10 and hop_pattern['avg_dwell_time'] < 0.01  # BT hops every 625us

        hop_pattern['is_bluetooth_like'] = is_bt_like
        hop_pattern['pattern_id'] = pattern_id

        if pattern_id not in hackrf_hop_sequences:
            hackrf_hop_sequences[pattern_id] = hop_pattern
            add_log(f"HackRF detected potential piconet (pattern: {pattern_id}, channels: {len(active_channels)})", "INFO")

            # Try to correlate with existing Ubertooth data
            if ubertooth_data:
                for lap, piconet in ubertooth_data.items():
                    # Check if channels overlap
                    ut_channels = piconet.get('channels', set())
                    overlap = len(set(hop_pattern['channels']) & ut_channels)
                    if overlap > 5:
                        hop_pattern['correlated_lap'] = lap
                        hop_pattern['correlation_strength'] = overlap / len(ut_channels) if ut_channels else 0
                        add_log(f"HackRF pattern {pattern_id} correlates with Ubertooth LAP {lap}", "INFO")
...
def analyze_bluetooth_spectrum():
    """
    Analyze the current spectrum data to identify Bluetooth activity
    ...
    if not hackrf_channel_activity:
        return analysis

    # Calculate statistics
    powers = [a['avg_power'] for a in hackrf_channel_activity.values()]
    if powers:
        analysis['noise_floor'] = min(powers)
        analysis['peak_power'] = max(powers)

        # Find peak channel
        for ch, activity in hackrf_channel_activity.items():
            if activity['avg_power'] == analysis['peak_power']:
                analysis['peak_channel'] = ch
                break

        # Count active channels
        active = [a for a in hackrf_channel_activity.values() if a['active']]
        analysis['active_channels'] = len(active)
        analysis['channel_utilization'] = (len(active) / 79) * 100  # % of BT channels active

        # Estimate potential devices based on hopping patterns
        # Each piconet typically uses ~20-30 channels actively
        if len(active) > 5:
            analysis['potential_devices'] = max(1, len(active) // 20)

    # Generate spectrum summary (79 channels)
    for ch in range(79):
        if ch in hackrf_channel_activity:
            activity = hackrf_channel_activity[ch]
            analysis['spectrum_summary'].append({
                'channel': ch,
                'freq_mhz': 2402 + ch,
                'power': round(activity['avg_power'], 1),
...
def hackrf_sweep_thread():
    """Background thread to run hackrf_sweep and capture Bluetooth spectrum."""
    global hackrf_process, hackrf_running, hackrf_sweep_data, hackrf_channel_activity

    add_log("HackRF spectrum sweep starting", "INFO")

    try:
        # Build hackrf_sweep command
        cmd = [
            'hackrf_sweep',
            '-f', f"{HACKRF_CONFIG['FREQ_START'] // 1000000}:{HACKRF_CONFIG['FREQ_STOP'] // 1000000}",
            '-l', str(HACKRF_CONFIG['LNA_GAIN']),
            '-g', str(HACKRF_CONFIG['VGA_GAIN']),
            '-w', str(HACKRF_CONFIG['FREQ_BIN_WIDTH']),
        ]

        if HACKRF_CONFIG['ONE_SHOT']:
            cmd.append('-1')

        add_log(f"HackRF command: {' '.join(cmd)}", "DEBUG")
        ...
        emit_interval = 0.5  # Emit updates every 500ms

        while hackrf_running and hackrf_process.poll() is None:
            line = hackrf_process.stdout.readline()
            if line:
                sweep_result = parse_hackrf_sweep_line(line.strip())
                if sweep_result:
                    sweep_count += 1

                    # Check for hopping patterns periodically
                    if sweep_count % 100 == 0:
                        piconet = detect_piconets_from_hopping()
                        if piconet:
                            socketio.emit('hackrf_piconet_detected', piconet)

                    # Emit spectrum updates at regular intervals
                    current_time = time.time()
                    if current_time - last_emit_time >= emit_interval:
                        analysis = analyze_bluetooth_spectrum()

                        socketio.emit('hackrf_spectrum_update', {
                            'spectrum': analysis['spectrum_summary'],
                            'active_channels': analysis['active_channels'],
                            'peak_power': analysis['peak_power'],
                            'peak_channel': analysis['peak_channel'],
                            'channel_utilization': analysis['channel_utilization'],
                            'potential_devices': analysis['potential_devices'],
                            'timestamp': datetime.now().isoformat()
                        })
        ...
        add_log("HackRF tools not found - install hackrf package", "ERROR")
    except Exception as e:
...
def hackrf_targeted_scan_thread(target_bd_address=None):
    """
    Targeted scan mode that focuses on specific frequency patterns
    to assist in locating a specific device.
    """
    global hackrf_running

    add_log(f"HackRF targeted scan starting for {target_bd_address or 'any device'}", "INFO")

    try:
        # If we have a target and know its hopping pattern (from Ubertooth),
        # we can predict which channels to watch
        target_channels = set()

        if target_bd_address and ubertooth_data:
            # Look for matching piconet
            for lap, piconet in ubertooth_data.items():
                bd_partial = piconet.get('bd_partial', '')
                if target_bd_address.upper() in bd_partial.upper():
                    target_channels = piconet.get('channels', set())
                    add_log(f"Found target in Ubertooth data, watching {len(target_channels)} channels", "INFO")
                    break

        # Run hackrf_sweep focusing on target channels if known
        # Otherwise scan full BT spectrum
        while hackrf_running:
            # Periodic analysis for target
            analysis = analyze_bluetooth_spectrum()

            if target_channels:
                # Check if target channels are active
                active_target_channels = sum(
                    1 for ch in target_channels
                    if ch in hackrf_channel_activity and hackrf_channel_activity[ch]['active']
                )

                if active_target_channels > len(target_channels) * 0.3:  # 30% threshold
                    socketio.emit('hackrf_target_detected', {
                        'bd_address': target_bd_address,
                        'active_channels': active_target_channels,
                        'total_channels': len(target_channels),
                        'signal_strength': analysis['peak_power'],
                        'timestamp': datetime.now().isoformat()
                    })
        ...
        add_log(f"HackRF targeted scan error: {e}", "ERROR")


def start_hackrf(mode='spectrum', target_bd=None):
    """
    Start the HackRF scanner.

    Args:
        mode: 'spectrum' for full spectrum sweep, 'targeted' for specific device
        target_bd: BD address for targeted mode
    """
    global hackrf_thread, hackrf_running, hackrf_sweep_data, hackrf_channel_activity
    global hackrf_piconet_data, hackrf_hop_sequences

    if hackrf_running:
        return False, "HackRF already running"

    if not check_hackrf_available():
        return False, "HackRF tools not available"

    # Clear previous data
    hackrf_sweep_data = {}
    hackrf_channel_activity = {}
    hackrf_piconet_data = {}
    hackrf_hop_sequences = {}
    hackrf_running = True

    if mode == 'targeted' and target_bd:
        hackrf_thread = threading.Thread(
            target=hackrf_targeted_scan_thread,
            args=(target_bd,),
            daemon=True
        )
    else:
        hackrf_thread = threading.Thread(target=hackrf_sweep_thread, daemon=True)

    hackrf_thread.start()

    # Also start the sweep thread if in targeted mode (need spectrum data)
    if mode == 'targeted':
        sweep_thread = threading.Thread(target=hackrf_sweep_thread, daemon=True)
        sweep_thread.start()

    return True, f"HackRF scanner started in {mode} mode"


def stop_hackrf():
    """Stop the HackRF scanner."""
    global hackrf_running, hackrf_process

    if not hackrf_running:
        return False, "HackRF not running"
    ...
    return True, "HackRF scanner stopping"


def get_hackrf_spectrum_data():
    """Get the current spectrum data for the Bluetooth band."""
    return {
        'sweep_data': dict(list(hackrf_sweep_data.items())[-1000:]),  # Last 1000 freq bins
        'channel_activity': {k: {**v, 'channels': list(v.get('channels', []))}
                            for k, v in hackrf_channel_activity.items()},
        'running': hackrf_running,
        'config': HACKRF_CONFIG
    }


def get_hackrf_piconets():
    """Get detected piconets and hopping patterns."""
    return {
        'piconets': list(hackrf_piconet_data.values()),
        'hop_sequences': list(hackrf_hop_sequences.values()),
        'channel_activity_summary': {
            'total_channels': 79,
            'active_channels': sum(1 for a in hackrf_channel_activity.values() if a.get('active')),
            'utilization': (sum(1 for a in hackrf_channel_activity.values() if a.get('active')) / 79) * 100
        }
    }


def correlate_hackrf_with_bluetooth_adapters():
    """
    Correlate HackRF spectrum data with devices detected by standard
    Bluetooth adapters for enhanced identification.
    """
    correlation_results = []

    for bd_address, device in devices.items():
        correlation = {
            'bd_address': bd_address,
            'device_name': device.get('name', 'Unknown'),
            'bt_adapter_rssi': device.get('rssi', -100),
            'hackrf_correlation': None,
            'estimated_distance': None
        }

        # Check if device is in Ubertooth piconet data
        if ubertooth_data:
            for lap, piconet in ubertooth_data.items():
                bd_partial = piconet.get('bd_partial', '')
                if bd_address.upper().endswith(bd_partial.upper().replace('??:', '')):
                    correlation['ubertooth_lap'] = lap
                    correlation['ubertooth_channels'] = list(piconet.get('channels', []))

                    # Check HackRF channel activity for these channels
                    if hackrf_channel_activity:
                        active_in_hackrf = sum(
                            1 for ch in piconet.get('channels', set())
                            if ch in hackrf_channel_activity and hackrf_channel_activity[ch]['active']
                        )
                        correlation['hackrf_active_channels'] = active_in_hackrf

                        # Get average power from HackRF for these channels
                        powers = [
                            hackrf_channel_activity[ch]['avg_power']
                            for ch in piconet.get('channels', set())
                            if ch in hackrf_channel_activity
                        ]
                        if powers:
                            correlation['hackrf_avg_power'] = sum(powers) / len(powers)

                            # Estimate distance using both HackRF and BT adapter RSSI
                            # Using path loss model: RSSI = TxPower - 10*n*log10(d) - losses
                            tx_power = -10  # Assumed Bluetooth TX power
                            path_loss_exp = 2.5  # Indoor environment

                            hackrf_rssi = correlation['hackrf_avg_power']
                            bt_rssi = correlation['bt_adapter_rssi']

                            # Weighted average of both RSSI sources
                            combined_rssi = (hackrf_rssi * 0.4 + bt_rssi * 0.6)
                            distance = 10 ** ((tx_power - combined_rssi) / (10 * path_loss_exp))

                            correlation['estimated_distance'] = round(distance, 2)
                            correlation['hackrf_correlation'] = 'strong' if active_in_hackrf > 5 else 'weak'
                    break
...
def export_hackrf_capture(filename=None):
    """Export HackRF capture data to a file for analysis."""
    if not filename:
        filename = f"hackrf_capture_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

    export_data = {
        'capture_time': datetime.now().isoformat(),
        'config': HACKRF_CONFIG,
        'spectrum_data': list(hackrf_sweep_data.items())[-5000:],  # Last 5000 bins
        'channel_activity': {k: {**v, 'channels': list(v.get('channels', []))}
                            for k, v in hackrf_channel_activity.items()},
        'detected_piconets': list(hackrf_piconet_data.values()),
        'hop_sequences': list(hackrf_hop_sequences.values()),
        'correlation_with_devices': correlate_hackrf_with_bluetooth_adapters()
    }

    filepath = os.path.join(INSTALL_DIR, 'data', filename)
...
@login_required
def hackrf_status():
    """Get HackRF device status and info."""
    info = get_hackrf_info()
    info['running'] = hackrf_running
    info['piconets_detected'] = len(hackrf_piconet_data)
    info['hop_patterns_detected'] = len(hackrf_hop_sequences)
    info['active_channels'] = sum(1 for a in hackrf_channel_activity.values() if a.get('active'))
    return jsonify(info)


@app.route('/api/hackrf/start', methods=['POST'])
@login_required
def start_hackrf_route():
    """Start the HackRF spectrum scanner."""
    data = request.get_json() or {}
    mode = data.get('mode', 'spectrum')
    target_bd = data.get('target_bd')
    success, message = start_hackrf(mode=mode, target_bd=target_bd)
    return jsonify({'status': 'started' if success else 'error', 'message': message})


@app.route('/api/hackrf/stop', methods=['POST'])
@login_required
def stop_hackrf_route():
    """Stop the HackRF scanner."""
    success, message = stop_hackrf()
    return jsonify({'status': 'stopped' if success else 'error', 'message': message})


@app.route('/api/hackrf/spectrum')
@login_required
def get_hackrf_spectrum():
    """Get current spectrum data from HackRF."""
    analysis = analyze_bluetooth_spectrum()
    return jsonify({
        'spectrum': analysis['spectrum_summary'],
        'active_channels': analysis['active_channels'],
...
@app.route('/api/hackrf/piconets')
@login_required
def get_hackrf_piconets_route():
    """Get detected piconets and hopping patterns from HackRF."""
    return jsonify(get_hackrf_piconets())


@app.route('/api/hackrf/channels')
@login_required
def get_hackrf_channels():
    """Get detailed channel activity data."""
    channels = []
    for ch in range(79):
        if ch in hackrf_channel_activity:
            activity = hackrf_channel_activity[ch]
            channels.append({
                ...
                'max_power': round(activity.get('max_power', -100), 1),
                'min_power': round(activity.get('min_power', -100), 1),
                'avg_power': round(activity.get('avg_power', -100), 1),
                'samples': activity.get('samples', 0),
                'active': activity.get('active', False),
                'last_active': activity.get('last_active'),
                'hop_count': activity.get('hop_count', 0)
            })
        else:
            channels.append({
                'channel': ch,
...
@app.route('/api/hackrf/correlation')
@login_required
def get_hackrf_correlation():
    """Get correlation data between HackRF and standard Bluetooth adapters."""
    correlations = correlate_hackrf_with_bluetooth_adapters()
    return jsonify({
        'correlations': correlations,
        'device_count': len(correlations),
        'correlated_count': sum(1 for c in correlations if c.get('hackrf_correlation')),
        'timestamp': datetime.now().isoformat()
    })


@app.route('/api/hackrf/config', methods=['GET', 'POST'])
@login_required
def hackrf_config():
    """Get or update HackRF configuration."""
    global HACKRF_CONFIG

    if request.method == 'GET':
        return jsonify(HACKRF_CONFIG)

    # POST - update configuration
    data = request.get_json() or {}

    # Validate and update allowed config options
    allowed_keys = [
        'FREQ_START', 'FREQ_STOP', 'FREQ_BIN_WIDTH',
        'LNA_GAIN', 'VGA_GAIN', 'SAMPLE_RATE',
        'ACTIVITY_THRESHOLD', 'HOP_DETECT_WINDOW', 'MIN_HOP_CHANNELS'
    ]
    ...
    for key in allowed_keys:
        if key in data:
            try:
                # Validate ranges
                if key == 'LNA_GAIN' and not (0 <= data[key] <= 40):
                    continue
                if key == 'VGA_GAIN' and not (0 <= data[key] <= 62):
                    continue
                if key in ['FREQ_START', 'FREQ_STOP']:
                    # Must be in valid frequency range
                    if not (1000000 <= data[key] <= 6000000000):
                        continue

                HACKRF_CONFIG[key] = data[key]
                updated.append(key)
            except (TypeError, ValueError):
                continue

    add_log(f"HackRF config updated: {updated}", "INFO")
    return jsonify({
        'status': 'updated',
...
@app.route('/api/hackrf/clear', methods=['POST'])
@login_required
def clear_hackrf_data():
    """Clear all HackRF captured data."""
    global hackrf_sweep_data, hackrf_channel_activity, hackrf_piconet_data, hackrf_hop_sequences
    ...
    add_log("HackRF data cleared", "INFO")
    return jsonify({'status': 'cleared'})


@app.route('/api/hackrf/export', methods=['POST'])
@login_required
def export_hackrf_data():
    """Export HackRF capture data to a file."""
    data = request.get_json() or {}
    filename = data.get('filename')

    try:
        filepath = export_hackrf_capture(filename)
        return jsonify({
            'status': 'exported',
            'filepath': filepath,
...
@app.route('/api/hackrf/targeted', methods=['POST'])
@login_required
def start_hackrf_targeted():
    """Start targeted HackRF scan for a specific device."""
    data = request.get_json() or {}
    target_bd = data.get('target_bd')

    if not target_bd:
        return jsonify({'status': 'error', 'message': 'target_bd required'}), 400

    success, message = start_hackrf(mode='targeted', target_bd=target_bd)
    return jsonify({'status': 'started' if success else 'error', 'message': message})


@app.route('/api/hackrf/waterfall')
@login_required
def get_hackrf_waterfall():
    """Get waterfall display data (time-frequency-power matrix)."""
    # Build waterfall data from recent sweep data
    waterfall = []
    sorted_freqs = sorted(hackrf_sweep_data.keys())

    # Get last N frequency bins for waterfall
    max_bins = 500
    freq_subset = sorted_freqs[-max_bins:] if len(sorted_freqs) > max_bins else sorted_freqs

    for freq in freq_subset:
        data = hackrf_sweep_data[freq]
        bt_channel = freq_to_bt_channel(freq)
        waterfall.append({
            'freq': freq,
...
@app.route('/api/hackrf/hop_patterns')
@login_required
def get_hackrf_hop_patterns():
    """Get detected frequency hopping patterns."""
    patterns = []
    for pattern_id, pattern in hackrf_hop_sequences.items():
        patterns.append({
            'pattern_id': pattern_id,
            'channels': pattern.get('channels', []),
            'channel_count': pattern.get('channel_count', 0),
            'avg_dwell_time': pattern.get('avg_dwell_time', 0),
            'avg_power': pattern.get('avg_power', -100),
            'is_bluetooth_like': pattern.get('is_bluetooth_like', False),
            'correlated_lap': pattern.get('correlated_lap'),
...
@app.route('/api/hackrf/fusion')
@login_required
def get_hackrf_fusion():
    """
    Get fused intelligence from HackRF + Ubertooth + BT adapters.
    This combines all available data sources for comprehensive analysis.
    """
    fusion_data = {
        'timestamp': datetime.now().isoformat(),
        'sources': {
            'hackrf': hackrf_running,
            'ubertooth': ubertooth_running,
            'bt_adapters': len(active_radios.get('bluetooth', []))
        },
        'detected_piconets': [],
        'device_correlations': [],
        'spectrum_summary': analyze_bluetooth_spectrum(),
        'recommendations': []
    }

    # Combine Ubertooth piconets with HackRF hop patterns
    seen_patterns = set()

    # Add Ubertooth piconets
    for lap, piconet in ubertooth_data.items():
        piconet_entry = {
            'source': 'ubertooth',
            'lap': lap,
            'uap': piconet.get('uap'),
            'bd_partial': piconet.get('bd_partial'),
            'channels': list(piconet.get('channels', set())),
            'packet_count': piconet.get('packet_count', 0),
            'first_seen': piconet.get('first_seen'),
            'last_seen': piconet.get('last_seen')
        }

        # Check for HackRF correlation
        for pattern_id, pattern in hackrf_hop_sequences.items():
            if pattern.get('correlated_lap') == lap:
                piconet_entry['hackrf_correlation'] = pattern_id
                piconet_entry['hackrf_power'] = pattern.get('avg_power')
                seen_patterns.add(pattern_id)
                break

        fusion_data['detected_piconets'].append(piconet_entry)

    # Add HackRF-only patterns (not correlated with Ubertooth)
    for pattern_id, pattern in hackrf_hop_sequences.items():
        if pattern_id not in seen_patterns:
            fusion_data['detected_piconets'].append({
                'source': 'hackrf',
                'pattern_id': pattern_id,
                'channels': pattern.get('channels', []),
                'avg_power': pattern.get('avg_power'),
                'is_bluetooth_like': pattern.get('is_bluetooth_like', False),
                'detected_at': pattern.get('detected_at')
            })

    # Add device correlations
    fusion_data['device_correlations'] = correlate_hackrf_with_bluetooth_adapters()

    # Generate recommendations
    if not hackrf_running and not ubertooth_running:
        fusion_data['recommendations'].append({
            'type': 'enable_scanner',
            'message': 'Enable HackRF or Ubertooth for deeper spectrum analysis'
        })

    if hackrf_running and len(hackrf_hop_sequences) > 0 and not ubertooth_running:
        fusion_data['recommendations'].append({
            'type': 'enable_ubertooth',
            'message': f'Detected {len(hackrf_hop_sequences)} hopping patterns - enable Ubertooth for LAP/UAP recovery'
        })

    channel_util = fusion_data['spectrum_summary'].get('channel_utilization', 0)
    if channel_util > 50:
        fusion_data['recommendations'].append({
            'type': 'high_activity',
            'message': f'High Bluetooth activity detected ({channel_util:.1f}% utilization) - good conditions for piconet detection'
        })
